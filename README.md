...
| [`critique`](skills/critique/) | Reviews a pull request on three separate axes (Standards, Spec, Policy & Docs) in parallel sub-agents, then posts the findings as inline review comments and submits the review. Discovers the repo's own conventions, policy doc, and ADRs at review time, so it works on any repository. | When you want a PR reviewed: pass its URL and say "critique this PR" or "review this PR". Add "dry run" to stage the comments without submitting. |
| [`cutting-releases`](skills/cutting-releases/) | Turns everything merged since the last release tag into a CHANGELOG entry, a GitHub release, and a Slack announcement, deploys to the target environment, and moves the shipped Linear tickets. **TPN-specific** — encodes the AnunțReal release pipeline. | When cutting or announcing a release: "cut a release", "release v1.1.x to dev/prod", "deploy to prod", "publish the changelog". |
...
| [`file-a-bug`](skills/file-a-bug/) | Files a bug as a Linear ticket to the TPN standard: nine required fields (including severity and frequency), a mandatory Linear project, and interactive multiple-choice questions for anything missing. Shows the draft before it creates anything. | When something is broken and needs tracking: "file a bug", "report this bug", "log a ticket for this", "raise a defect", "write this up as a ticket". |
| [`repro`](skills/repro/) | Validates a bug against a running environment: reads the Linear ticket, reproduces it in a real browser via the Claude in Chrome extension, and posts a verdict — solved, not solved, partially solved (or blocked) — as a comment on the ticket. Strictly black-box: the ticket and the host are the only sources, never the code. | When you need to know whether a fix actually works on a host: "is ENG-123 really fixed on dev", "validate this bug on staging", "re-test this ticket", "QA ENG-482". Needs a ticket id and a host URL. |
| [`ui-ux-pro-max`](skills/ui-ux-pro-max/) | A searchable design knowledge base (80+ styles, 160+ palettes, font pairings, UX rules, chart picks across 17 web/mobile stacks) plus a Python query tool that recommends a full design system and flags anti-patterns. | When building or reviewing UI: designing a page or component, picking a style/color/typography system, adding dark mode/animation/responsive layout, or checking UI code for accessibility and visual consistency. |
